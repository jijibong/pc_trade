...
class DesktopTabState {
  ...
  final ScrollPosController scrollController = ScrollPosController(itemCount: 0);
...
class DesktopTabController {
  ...
  DesktopTabController({required this.tabType, this.onRemoved, this.onSelected});
    ...
    state.update((val) {
      if (val != null) {
        ...
        Future.delayed(const Duration(milliseconds: 100), (() {
          ...
          if (val.scrollController.hasClients && val.scrollController.itemCount > index) {
            val.scrollController.scrollToItem(index, center: false, animate: true);
          }
        }));
      }
    });
    if ((isDesktop && (bind.isIncomingOnly() || bind.isOutgoingOnly())) || callOnSelected) {
  ...
  bool jumpToByKey(String key, {bool callOnSelected = true}) => jumpTo(state.value.tabs.indexWhere((tab) => tab.key == key), callOnSelected: callOnSelected);
...
typedef TabBuilder = Widget Function(String key, Widget icon, Widget label, TabThemeConf themeConf);
...
class DesktopTab extends StatefulWidget {
  ...
  final _scrollDebounce = Debouncer(delay: const Duration(milliseconds: 50));
...
class _DesktopTabState extends State<DesktopTab> with MultiWindowListener, WindowListener {
  final _saveFrameDebounce = Debouncer(delay: const Duration(seconds: 1));
  ...
  Widget Function(Widget pageView)? get pageViewBuilder => widget.pageViewBuilder;
  TabMenuBuilder? get tabMenuBuilder => widget.tabMenuBuilder;
  Widget? get tail => widget.tail;
  Future<bool> Function()? get onWindowCloseButton => widget.onWindowCloseButton;
  ...
  Color? get unSelectedTabBackgroundColor => widget.unSelectedTabBackgroundColor;
  ...
  bool get isMainWindow => tabType == DesktopTabType.main || tabType == DesktopTabType.install;
      ...
      if (isMainWindow) {
        windowManager.isMaximized().then((maximized) {
          if (stateGlobal.isMaximized.value != maximized) {
            WidgetsBinding.instance.addPostFrameCallback((_) => setState(() => stateGlobal.setMaximized(maximized)));
          }
        });
      } else {
  ...
  void onWindowClose() async {
    ...
    macOSWindowClose(
      ...
      _macOSCheckRestoreTimer = Timer.periodic(const Duration(milliseconds: 30), (timer) async {
        _macOSCheckRestoreCounter++;
        if (!await checkFullscreen() || _macOSCheckRestoreCounter >= 30) {
          _macOSCheckRestoreTimer?.cancel();
          _macOSCheckRestoreTimer = null;
          Timer(const Duration(milliseconds: 700), () async => await closeFunc());
        }
      });
    }
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      Obx(() {
        if (stateGlobal.showTabBar.isTrue && !(kUseCompatibleUiMode && isHideSingleItem())) {
      ...
      Expanded(child: pageViewBuilder != null ? pageViewBuilder!(_buildPageView()) : _buildPageView())
    ]);
  }

  Widget _buildBlock({required Widget child}) {
    // if (blockTab != null) {
    // return buildRemoteBlock(child: child, block: blockTab!, use: canBeBlocked, mask: tabType == DesktopTabType.main);
    // } else {
    return child;
    // }
  }
        ...
        child: Obx(() => PageView(
            ...
            children: () {
              ...
              if (tabLen == _tabWidgets.length) {
                return _tabWidgets;
              } else if (_tabWidgets.isNotEmpty && tabLen == _tabWidgets.length + 1) {
  ...
  bool isHideSingleItem() {
    return state.value.tabs.length == 1 && (controller.tabType == DesktopTabType.main || controller.tabType == DesktopTabType.install);
  }
                ...
                onTap: !(bind.isIncomingOnly() && isInHomePage()) && showMaximize
                    ? () {
                        ...
                        if (elapsed < bind.getDoubleClickTime()) {
                          // onDoubleTap
                          // toggleMaximize(isMainWindow).then((value) => stateGlobal.setMaximized(value));
                        }
                      }
                    : null,
                  ...
                  children: [
                    ...
                    Offstage(
                      ...
                      child: Row(children: [
                        Offstage(
                          offstage: !showLogo,
                          child: Image.asset('assets/images/jmaster.ico', width: 16, height: 16),
                        ),
                        Offstage(
                            offstage: !showTitle,
                            child: Text(
                            ...
                            onPointerSignal: (e) {
                              if (e is PointerScrollEvent) {
                                ...
                                _scrollDebounce.call(() {
                                  sc.animateTo(sc.offset + e.scrollDelta.dy, duration: const Duration(milliseconds: 200), curve: Curves.ease);
                                });
                              }
                            },
                            child: _ListView(
...
class WindowActionPanelState extends State<WindowActionPanel> {
  ...
  List<String> existingInvisibleTab() {
    return widget.invisibleTabKeys.where((key) => widget.tabController.state.value.tabs.any((tab) => tab.key == key)).toList();
  }
      ...
      children: [
        Obx(() {
          if (showTabDowndown() && existingInvisibleTab().isNotEmpty) {
            return _TabDropDownButton(controller: widget.tabController, labelGetter: widget.labelGetter, tabkeys: existingInvisibleTab());
          } else {
            return const Offstage();
          }
        }),
        ...
        if (!kUseCompatibleUiMode)
          Row(
            children: [
              ...
              //   Obx(() => ActionIcon(
              //         icon: stateGlobal.isMaximized.isTrue ? FluentIcons.chrome_restore : FluentIcons.grid_view_large,
    ...
    return Obx(() => ListView(
        ...
        children: isHideSingleItem()
            ...
            : state.value.tabs.asMap().entries.map((e) {
                final index = e.key;
                final tab = e.value;
                final label = labelGetter == null ? Rx<String>(tab.label) : labelGetter!(tab.label);
...
class _TabState extends State<_Tab> with RestorationMixin {
  ...
  Widget _buildTabContent() {
    bool showIcon = widget.selectedIcon != null && widget.unselectedIcon != null;
    ...
    final icon = Offstage(
        ...
        child: Icon(
          ...
          size: _kIconSize,
          // color: isSelected ? MyTheme.tabbar(context).selectedTabIconColor : MyTheme.tabbar(context).unSelectedTabIconColor,
        ).paddingOnly(right: 5));
    final labelWidget = Obx(() {
      return ConstrainedBox(
          constraints: BoxConstraints(maxWidth: widget.maxLabelWidth ?? 200),
          child: Tooltip(
            message: widget.tabType == DesktopTabType.main ? '' : widget.label.value,
            child: Text(
              widget.label.value,
              textAlign: TextAlign.center,
              // style: TextStyle(color: isSelected ? MyTheme.tabbar(context).selectedTextColor : MyTheme.tabbar(context).unSelectedTextColor),
  ...
  Widget build(BuildContext context) {
    bool isSelected = widget.index == widget.selected;
    bool showDivider = widget.index != widget.selected - 1 && widget.index != widget.selected;
      ...
      child: InkWell(
        ...
        child: Container(
            decoration: isSelected && widget.selectedBorderColor != null
                ...
                : null,
            child: Container(
              color: isSelected ? widget.selectedTabBackgroundColor : widget.unSelectedTabBackgroundColor,
              child: Row(
                children: [
                  SizedBox(
                      // _kTabBarHeight also displays normally
                      height: _showTabBarBottomDivider(widget.tabType) ? _kTabBarHeight - 1 : _kTabBarHeight,
...
class _ActionIconState extends State<ActionIcon> {
  ...
  Widget build(BuildContext context) {
    return Tooltip(
      ...
      child: InkWell(
        hoverColor: const Color.fromARGB(255, 196, 43, 28),
        ...
        child: SizedBox(
            ...
            //     () => Icon(
            //       widget.icon,
            //       color: hover.value && widget.isClose ? Colors.white : MyTheme.tabbar(context).unSelectedIconColor,
            //       size: widget.iconSize,
            //     ),
            //   ),
            ),
...
class AddButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ActionIcon(
        message: 'New Connection', icon: FluentIcons.add, onTap: () => rustDeskWinManager.call(WindowType.Main, kWindowMainWindowOnTop, ""), isClose: false);
  }
}
...
class _TabDropDownButton extends StatefulWidget {
  ...
  const _TabDropDownButton({required this.controller, required this.tabkeys, this.labelGetter});
...
class _TabDropDownButtonState extends State<_TabDropDownButton> {
  ...
  @override
  Widget build(BuildContext context) {
    List<String> sortedKeys = widget.controller.state.value.tabs.where((e) => widget.tabkeys.contains(e.key)).map((e) => e.key).toList();
    return ActionIcon(
      onTapDown: (details) {
      ...
      onTap: () {
        showMenu(
          ...
          items: sortedKeys.map((e) {
            var label = e;
            final tabInfo = widget.controller.state.value.tabs.firstWhereOrNull((element) => element.key == e);
            if (tabInfo != null) {
              label = tabInfo.label;
            }
            ...
            var index = widget.controller.state.value.tabs.indexWhere((t) => t.key == e);
            label = '${index + 1}. $label';
              ...
              child: MouseRegion(
                ...
                child: Row(
                  children: [
                    ...
                    Obx(
                      () {
                        if (tabInfo?.onTabCloseButton != null && menuHover.value) {
                          return InkWell(
                              onTap: () {
                              ...
                              child: MouseRegion(
                                  ...
                                  onHover: (event) => setState(() => btnHover.value = true),
                                  onExit: (event) => setState(() => btnHover.value = false),
                                  child: Icon(Icons.close, color: btnHover.value ? Colors.red : null)));
                        } else {
                          return const Offstage();
                        }
                      },
...
class TabbarTheme extends ThemeExtension<TabbarTheme> {
  ...
  @override
  ThemeExtension<TabbarTheme> lerp(ThemeExtension<TabbarTheme>? other, double t) {
    if (other is! TabbarTheme) {
      return this;
    }
    return TabbarTheme(
      selectedTabIconColor: Color.lerp(selectedTabIconColor, other.selectedTabIconColor, t),
