...
class TradeWebSocketServer {
    ...
    switch (c.option) {
      case Option.REQ_OPT_AUTH:
        ...
        if (subResp.code == 0) {
          ...
          dispose();
          InfoBarUtils.showWarningBar("身份令牌已过期，请重新登录");
  ...
  void checkNeedBackHand(CDFYRspFillDetail fill) async {
    ...
    //
    //   DealServer.addOrder(ExchangeNo, CommodityNo, ContractNo, CommodityType, OrderType, TimeInForce, ExpireTime, OrderSide, OrderPrice, StopPrice, OrderQty,
    //       PositionEffect, ClientOrderId);
    //
